...
{
    /*FIXME: write this*/
	struct Task* leftTmp = (struct Task*) left;
	struct Task* rightTmp = (struct Task*) right;
	if (leftTmp->priority > rightTmp->priority)
		return 1;
	else if (leftTmp->priority == rightTmp->priority)
		return 0;
	else
		return -1;
