...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
  //Compare two nodes using getDynArr
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
	...
	while (current != 0)
	{
		parent = (current - 1) / 2;      //size -1/ 2  parent formula

		if (compare(getDynArr(heap, current), getDynArr(heap, parent)) == -1)
		{
			swapDynArr(heap, parent, current);
			current = parent;
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  /* FIXME: Write this */
	...
	//position * 2  + 1 for left   position * 2 + 2 for right
	int leftChild = pos * 2 + 1;
	int rightChild = pos * 2 + 2;

	//create variables that will become 1 if children < max 
	int smallest = -1;
...
void removeMinHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write this */
	...
	heap->data[0] = getDynArr(heap, lastNode);
...
void _buildHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write This */
	...
	//first leaf of heap is always size/2 -1
	int leaf = (sizeDynArr(heap) / 2) - 1;

	for (int i = leaf; i >= 0; i--)
		_adjustHeap(heap, sizeDynArr(heap), i, compare);
}
/*
