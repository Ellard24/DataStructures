...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	//Check if source and destination are the same thing
	if (source == destination)
		return 1;

	struct cirListDeque *depthStack = malloc(sizeof(struct cirListDeque));
	struct Vertex *current;
	initCirListDeque(depthStack);
	addFrontCirListDeque(depthStack, source);
	clearVisited(g);

	while (depthStack->size != 0)
	{
		//Set current to front of the stack before removing from stack
		current = frontCirListDeque(depthStack);
		removeFrontCirListDeque(depthStack);


		//Check front of stack to destination
		...
		/*If current vertex isnt visisted, set to visited. Then check all neighbors of 
		current vertex, and then add to front of stack if not visited.(Do not set their isVisited to 1 yet)*/
		if (current->isVisited != 1)
		{
			current->isVisited = 1;
			...
			for (int i = 0; i < current->numNeighbors;i++)
			{
				if (current->neighbors[i]->isVisited != 1)
					addFrontCirListDeque(depthStack, current->neighbors[i]);
			}
		}
	...
	//free(depthStack);
	if (isEmptyCirListDeque(depthStack))
		free(depthStack);
	else
	{
		while (!isEmptyCirListDeque(depthStack))
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct cirListDeque *breadthQueue = malloc(sizeof(struct cirListDeque));
	struct Vertex *current;
	initCirListDeque(breadthQueue);
	addBackCirListDeque(breadthQueue, source);
	clearVisited(g);

	//Same process as DFS, just with removing from front
	while (breadthQueue->size != 0)
	{
		current = frontCirListDeque(breadthQueue);
		...
		if (current->isVisited != 1)
		{
			...
			{
				if (current->neighbors[i]->isVisited != 1)
					addBackCirListDeque(breadthQueue, current->neighbors[i]);
			}
	...
	if (isEmptyCirListDeque(breadthQueue))
		free(breadthQueue);
	else
	{
		while (!isEmptyCirListDeque(breadthQueue))
